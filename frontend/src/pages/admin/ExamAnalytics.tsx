...
import type { ExamAnalyticsItem } from '../../services/admin/adminApiService';
...
export function ExamAnalytics() {
  const [examData, setExamData] = useState<ExamAnalyticsItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedExam, setSelectedExam] = useState<ExamAnalyticsItem | null>(null);
  ...
  useEffect(() => {
    const fetchExamAnalytics = async () => {
      ...
      setError(null);
      try {
        const data = await AdminApiService.getExamAnalytics();
        setExamData(data);
        if (data.length > 0) setSelectedExam(data[0]);
      } catch (err: any) {
        setError(err.message || 'Failed to fetch exam analytics');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error) {
    return (
      <div className="card-surface p-6 border border-danger/20">
        <p className="text-danger font-medium">Error loading exam analytics</p>
